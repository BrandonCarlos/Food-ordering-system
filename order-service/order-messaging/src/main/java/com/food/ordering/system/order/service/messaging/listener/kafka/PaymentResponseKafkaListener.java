...
@Slf4j //this is to create local variable
@Component
public class PaymentResponseKafkaListener implements KafkaConsumer<PaymentResponseAvroModel> {//KafkaConsumer came from Kafka-consumer-module, let's consume PaymentResponseAvroModel as data

    private final PaymentResponseMessageListener paymentResponseMessageListener; //one of the input ports in the domain layer
    ...
    public void receive(
            ...
            @Header(KafkaHeaders.RECEIVED_MESSAGE_KEY) List<String> keys,//These values came from Kafka
        ...
        messages.forEach(paymentResponseAvroModel -> {
            if(PaymentStatus.COMPLETED == paymentResponseAvroModel.getPaymentOrderStatus()) {
                log.info("Processing successful payment to order id: {}", paymentResponseAvroModel.getOrderId());
